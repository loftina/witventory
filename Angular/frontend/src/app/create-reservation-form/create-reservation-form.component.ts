...
export class CreateReservationFormComponent implements OnInit {
	...
	getAvailableRanges(date, starting_dates, ending_dates) {
		...
		if (starting_dates.length == 0) {
			return [{ 'start': new Date(`${date.getYear() + 1900}-${this.pad(date.getMonth()+1, 2)}-${this.pad(date.getDate(), 2)}T00:00:00.000-05:00`), 'end': new Date(`${date.getYear() + 1900}-${this.pad(date.getMonth()+1, 2)}-${this.pad(date.getDate(), 2)}T24:00:00.000-05:00`), 'isCollapsed': false }]
		}

		var first_start = starting_dates.shift()
		var last_end = ending_dates.pop()

		if (first_start > new Date(`${first_start.getYear() + 1900}-${this.pad(first_start.getMonth()+1, 2)}-${this.pad(first_start.getDate(), 2)}T00:00:00.000-05:00`)) {
			available_dates.push({ 'start': new Date(`${first_start.getYear() + 1900}-${this.pad(first_start.getMonth()+1, 2)}-${this.pad(first_start.getDate(), 2)}T00:00:00.000-05:00`), 'end': first_start, 'isCollapsed': false })
		}
		...
		if (last_end < new Date(`${first_start.getYear() + 1900}-${this.pad(first_start.getMonth()+1, 2)}-${this.pad(first_start.getDate(), 2)}T24:00:00.000-05:00`)) {
			available_dates.push({ 'start': last_end, 'end': new Date(`${first_start.getYear() + 1900}-${this.pad(first_start.getMonth()+1, 2)}-${this.pad(first_start.getDate(), 2)}T24:00:00.000-05:00`), 'isCollapsed': false })
		}
	...
	onDateSelect(date) {
		...
		var start_dates = []
		var end_dates = []
		this.http.get(`${this.API}/reservations/1`, {params: {start: `${date.year}-${padded_month}-${padded_day}T00:00:00.000-05:00`, end: `${date.year}-${padded_month}-${padded_day}T24:00:00.000-05:00`}})
			.subscribe((reservationResp: any) => {
				...
				if (true) {
					this.available_dates = this.getAvailableRanges(new Date(`${date.year}-${padded_month}-${padded_day}T00:00:00.000-05:00`), start_dates.sort(), end_dates.sort())
